...
def pogoda_msk_today():
    ...
    while countA in range(1, 9):
        weather = soup.select_one(f'body > section.content.wrap > div.content-column.column1 > section:nth-child(3) > div > div > div > div > div.widget-row.widget-row-icon > div:nth-child({countA}) > div')['data-text']
    ...
    data_msk = {'temperature' : data_temperature, 'precipitation_mm' : data_precipitation_mm, 'precipitation' : data_precipitation, 'wind' : data_wind, 'wind_speed' : data_wind_speed, 'atmospheric_pressure' : data_atmo_pressure, 'humidity' : data_humidity}
    with open('data_pogoda', 'w', encoding = 'utf-8') as f: 
        f.write(json.dumps(data_msk, ensure_ascii = False, indent = 4))

def prognos():
    response = requests.get(url_prognos_10days, headers={"User-Agent" : "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36"})
    soup = bs(response.text, 'lxml')
    ...
    while countA in range(1, 11):
        weather = soup.select_one(f'body > section.content.wrap > div.content-column.column1 > section:nth-child(2) > div > div > div > div > div.widget-row.widget-row-icon > div:nth-child({countA}) > div')['data-text']
    ...
    data_msk_10days = {'temperature' : data_temperature_10days, 'precipitation' : data_precipitation_10days, 'precipitation_mm' : data_precipitation_mm_10days, 'wind' : data_wind_10days, 'wind_speed' : data_wind_speed_10days, 'atmospheric_pressure' : data_atmospheric_pressure_10days, 'humidity' : data_humidity_10days}
